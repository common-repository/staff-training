...
if ( is_user_logged_in() ) {
    ...
    $user_id = 1;
    // Assuming a default user ID when not logged in
}
    ...
    ?>
                                                <tr>
                                                    <td class="people-name">
                                                        <?php 
    echo esc_html( $srtv['name'] );
    ?>
                                                    </td>
                                                    <td>                  
                                                    <?php 
    echo esc_html( $srtv['score'] );
    ?>
                                                    </td>  
...
?>  
                ...
                <a href="<?php 
echo esc_url( site_url() ) . '/wp-login.php?action=lostpassword';
?>">
                ...
                <a href="<?php 
echo esc_url( wp_logout_url( site_url() . '/wp-admin/' ) );
?>">
                    <span class="lg-text">
                        Logout
                    </span>
...
if ( !empty( $completed_learning_modules ) ) {
    ...
    foreach ( $completed_learning_modules as $key => $value ) {
        ...
        $score_sql = $wpdb->get_results( $wpdb->prepare( "SELECT quizid, score FROM {$wpdb->prefix}quiz_user_details WHERE userid = %d AND quizid IN (SELECT quizid FROM {$wpdb->prefix}quiz_details WHERE sectionid = %d)", $user_id, $value['id'] ), ARRAY_A );
...
<?php 
        ...
        ?>
                        ...
                        <a href="javascript:void(0);" class="show-popup accordian-text" data-showpopup="<?php 
        echo esc_attr( $key );
        ?>">
                            <?php 
        ...
        ?>
                    ...
                    <div class="overlay-content popup" id="cmplt_<?php 
        echo esc_attr( $key );
        ?>">
                        <button class="close-btn">x</button>
        ...
        ?></h3>
                                    ...
                                    <?php 
        echo esc_html( mystaff_training_get_current_steps( $value ) );
        ?>
        ...
        if ( !empty( $value['pages'] ) ) {
            ...
            foreach ( $value['pages'] as $key => $learning_subsec ) {
                ...
                echo '<li class="' . esc_attr( $complated_class ) . '"><a href="' . esc_url( $learning_subsec['sub_start_url'] . '?step=' . urlencode( $learning_subsec['sub_title'] ) ) . '">' . esc_html( $learning_subsec['sub_title'] ) . '</a></li>';
                $total_steps++;
            }
        }
        ?>
                            ...
                            <?php 
        if ( $avg == '' ) {
            echo esc_html( mystaff_training_get_progress_bar( $total_steps, $total_completed_step ) );
        } else {
            echo esc_html( mystaff_training_get_quiz_progress_bar( $avg ) );
        }
        ?>
                        </div>
                    </div>
...
if ( !empty( $results ) ) {
    ...
    foreach ( $results as $plist ) {
        ...
        ?>">
                                        <div class="image">
                                        <img src="<?php 
        echo esc_url( wp_get_attachment_image_src( $plist['image_icon'] )[0] );
        ?>" height="75px" width="75px"/>
                                        </div>
                                        <div class="info">
                                            <span><?php 
        echo esc_html( $plist['product_name'] );
        ?></span>
                                            <span><?php 
        echo esc_html( $plist['product_price'] );
        ?>pts</span>
                                        </div>
                                        <div class="purchase">
                                            <a href="javascript:void(0);" data-pid="<?php 
        echo esc_attr( $plist['id'] );
        ?>" data-price="<?php 
        echo esc_attr( $plist['product_price'] );
...
if ( empty( $completed_learning_modules ) || count( $completed_learning_modules ) == 0 && get_option( 'first_wlcm_message' ) != '' ) {
    ...
    if ( !empty( $completed_learning_modules ) && count( $completed_learning_modules ) > 0 && get_option( 'scnd_cmpltd_message' ) != '' ) {
        ...
        echo 'Welcome to staff training. Here’s where you’ll find the training courses available to you. Complete the sections below. Click START to begin or CONTINUE to resume where you left off.';
    }
}
?>
    ...
    foreach ( $learning_modules as $key => $user_assigned ) {
        if ( $user_assigned['active'] == 1 && $user_assigned['is_all_complated'] == '' ) {
            ...
            ?>
                                        ...
                                        <h3><?php 
            echo esc_html( $user_assigned['title'] );
            ?></h3>

                                        <?php 
            echo esc_html( mystaff_training_get_current_steps( $user_assigned ) );
            ?>
            ...
            if ( !empty( $user_assigned['pages'] ) ) {
                ...
                foreach ( $user_assigned['pages'] as $key => $learning_subsec ) {
                    ...
                    echo '<li ' . esc_attr( $complated_class ) . '><a href="' . esc_url( $learning_subsec['sub_start_url'] . '?step=' . urlencode( $learning_subsec['sub_title'] ) ) . '">' . esc_html( $learning_subsec['sub_title'] ) . '</a></li>';
                    $total_steps++;
                }
            }
            ?>
    ...
    foreach ( $learning_modules_unassigned as $key => $user_assigned ) {
        ...
        if ( $user_assigned['active'] == 1 && $user_assigned['is_all_complated'] == '' && ($user_assigned['cat'] == 'All' || $user_assigned['cat'] == $user_cat) ) {
            ...
            ?>" 
                            ...
                            <?php 
            echo ( esc_attr( $allowSelfAssign ) ? '' : 'disabled' );
            ?>
                            >
                            <?php 
            if ( $allowSelfAssign ) {
...
echo esc_url( site_url( '/dashboard' ) );
?>';
