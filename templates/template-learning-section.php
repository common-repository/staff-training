...
if ( isset( $_GET['action'] ) ) {
            ...
            <div class="wrapper">
                <form action="" method="" id="add_form" name="add_form">
                    <div class="top-section-details">
                        ...
                        <label for="section_icon"><img width="50px" src="<?php 
        echo esc_url( mystaff_training_plugin_dir_folder . '/icons-camera.png' );
        ?>"/></label>
    ...
    <?php 
        ...
        <div class="crud-learning-section">
            ...
            ?>"/></label>
                            ...
                            <input id="section_icon" name="section_icon_attachment" type="hidden" value="<?php 
            echo esc_attr( $get_data->image );
            ?>" />
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" name="section_title" placeholder="Title" value="<?php 
            echo esc_attr( $get_data->title );
            ?>" />
    ...
    <option value="All" <?php 
            echo esc_attr( ( $get_data->cat == 'All' ? 'selected' : '' ) );
            ?>>All</option>
    <option value="Installation" <?php 
            echo esc_attr( ( $get_data->cat == 'Installation' ? 'selected' : '' ) );
            ?>>Installation</option>
    <option value="Maintenance" <?php 
            echo esc_attr( ( $get_data->cat == 'Maintenance' ? 'selected' : '' ) );
            ?>>Maintenance</option>
    <option value="None" <?php 
            echo esc_attr( ( $get_data->cat == 'None' ? 'selected' : '' ) );
            ?>>None</option>
</select>
                        </div>
            ...
            if ( !empty( $learning_subsection ) ) {
                foreach ( $learning_subsection as $key => $item ) {
                    ...
                    $ql = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}quiz_details WHERE sectionid = %d AND subsection_title = %s", $id, $title ), ARRAY_A );
                    ...
                    ?>
                                ...
                                <div class="sections-fields">
                                    ...
                                    <div class="form-field">
                                        ...
                                        <input type="text" name="sub_section_title[]" placeholder="Subsection Title" required value="<?php 
                    echo esc_html( $item['sub_title'] );
                    ?>" id="sub_section_title_<?php 
                    echo esc_html( $key );
                    ?>"  <?php 
                    echo esc_html( $read_only );
                    ?> />
    ...
    <select name="sub_section_start_url[]" data-control="select2" style="width:100%;" class="form-select">
        <option value="<?php 
                    echo esc_attr( $selected_url );
                    ?>" selected><?php 
                    echo esc_html( $selected_title );
                    ?></option>
        ...
        <option value="<?php 
                    echo esc_attr( $item['sub_completed_url'] );
                    ?>" selected><?php 
                    echo esc_html( $selected_title );
                    ?></option>
        <?php 
            ...
            <?php 
                    ...
                    ?>
                                        <a class="quiz-blue-btn" href="admin.php?page=quiz-section&section_id=<?php 
                    echo esc_attr( $id );
                    ?>&sub_sec_title=<?php 
                    echo esc_attr( urlencode( $item['sub_title'] ) );
            ...
            ?>
                    ...
                    <div class="section-action-btn">
                        ...
                        <input type="submit" name="move_to_trash" value="Delete" data-id="<?php 
            echo esc_attr( $id );
            ?>"/>
    ...
    <?php 
        ...
        <div class="learning-page-content">
            ...
            <ul>
                ...
                <?php 
                echo esc_html( mystaff_training_get_all_users( $get_data ) );
                ?>
    ...
    $total_elements = count( $learning_sections );
    if ( $total_elements < 4 ) {
        ?>

        <a href="<?php 
        echo esc_url( admin_url( 'admin.php?page=learning_sections' ) );
        ?>&action=add">+ Add New Learning Section</a>
<?php 
    } else {
        ?>
    ...
    if ( !empty( $learning_sections ) ) {
        ?>
                    <li>
                        ...
                        <a class="assignedusers" href="#"><img src="<?php 
        echo esc_url( mystaff_training_plugin_dir_folder . '/images/gradcap.png' );
        ?>" width="40px"></a>
        ...
        foreach ( $learning_sections as $key => $learning_section ) {
            if ( $learning_section->is_trash == 0 ) {
                ?>
                        ...
                        <li id="<?php 
                echo esc_attr( $learning_section->id );
                ?>">
                            <div class="sort"><span class="dashicons dashicons-move"></span></div>
                            <div class="section-title"><?php 
                echo esc_html( $learning_section->title );
                ?></div>

                            <div class="section-action-btn">
                            <a href="<?php 
                echo esc_url( admin_url( 'admin.php?page=learning_sections' ) );
                ?>&action=edit&section_id=<?php 
                echo esc_attr( $learning_section->id );
                ?>">Edit</a>

                            <a href="<?php 
                echo esc_url( admin_url( 'admin.php?page=learning_sections' ) );
                ?>&action=assign_users&section_id=<?php 
                echo esc_attr( $learning_section->id );
                ?>">Assign</a>
                            ...
                            foreach ( $learning_modules as $key => $value ) {
                                ...
                                if ( $key == $learning_section->id ) {
                                    if ( $value['is_all_complated'] == 1 && $value['active'] == 1 ) {
                                        ...
                                        $completed += 1;
                                        $score_sql = $wpdb->get_results( $wpdb->prepare( "SELECT quizid, score FROM {$quiz_user} \n                                                                 WHERE userid = %d AND quizid IN (\n                                                                     SELECT quizid FROM {$quiztable} WHERE sectionid = %d\n                                                                 )", $uid, $value['id'] ), ARRAY_A );
                        ...
                        if(data == 'deleted'){
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    window.location.href = '<?php 
echo esc_url( admin_url( 'admin.php?page=trash-list' ) );
?>';
