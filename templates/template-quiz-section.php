...
<div class="wrapper">
    <div class="quiz-main-section">
        ...
        if (isset($_GET['quizid'])) {
            ...
            $list = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}quiz_details WHERE quizid = %d", $quizid));
        }


        if (!empty($list) && isset($list[0])) {
            $Questions = json_decode($list[0]->question_list, true);
            $total_elements = count($Questions);
        } else {
            $Questions = null;
        }
        ...
        <div class="title-wrap">
            <div class="backto-wrap">
                <a href="<?php
                            echo  esc_url(admin_url('admin.php') . '?page=learning_sections&action=edit&section_id=' . esc_attr(sanitize_text_field($_GET['section_id'])));
                            ?>" title="Back to Learning section" class="backtolearning">
                    <img src="<?php
                                echo  esc_url(mystaff_training_plugin_dir_folder . '/images/back.png');
                                ?>" />
                </a>
                <h1><?php
                    echo  esc_html(sanitize_text_field($_GET['sub_sec_title']));
                    ?> Quiz</h1>
            </div><!-- .backto-wrap -->
            <?php
            ...
            if (!empty($list)) {
            ?>
                <button class="btn  delete-quiz" data-quizid="<?php
                                                                echo  esc_attr(sanitize_text_field($_GET['quizid']));
                                                                ?>" title="Delete Quiz"><span class="dashicons dashicons-trash"></span></button>
            <?php
            }
        ...
        <form action="" id="quiz_ques_form" name="quiz_ques_form" method="">
            ...
            ?>
                ...
                <input type="hidden" name="quizid" value="<?php
                        ...
                        <div class="que-item">
                            <div class="ques-data">
                                <div class="ques-sr">
                                    ...
                                    <span class="question-no"><?php
                                                                echo  esc_html($key . '. ');
                                                                ?></span>
                                </div>
                                <input type="text" placeholder="Enter Question" name="ques_title[]" id="ques_title_<?php
                                                                                                                    echo  esc_attr($key);
                                                                                                                    ?>" value="<?php
                        echo  esc_attr($data['Question']);
                    ?>" class="ques-title-input" readonly />
                                <label class="ques-lbl"><?php
                                                        echo  esc_html($data['Question']);
                                                        ?></label>
                            ...
                            <div class="question-form">
                                ...
                                <div class="answer-wrap">
                                    ...
                                    ?>
                                        ...
                                        <div class="ans-list">
                                            <div class="ans-data">
                                                <input type="checkbox" name="quiz_answer[]" id="quiz_answer_<?php
                                                                                                            echo  esc_attr($k);
                                                                                                            ?>" <?php
                                        echo  esc_attr($selected);
                ?> />
                                                <label for="quiz_answer_<?php
                                                                        echo  esc_attr($k);
                                                                        ?>" name="answerlabel"><?php
                                        echo  esc_html($ans);
                                    ?></label>
                                                <input type="text" placeholder="Enter Answer" value="<?php
                                                                                                        echo  esc_attr($ans);
                                                                                                        ?>" name="answer_input[]" id="answer_input_<?php
                                                        echo  esc_attr($k);
                                                        ?>" class="answer-input" />
                                                <span class="error ans-error"></span>
                                            </div>
                                            <div class="action">
                                                <a href="javascript:void(0);" class="ans-del" data-key="<?php
                                                                                                        echo  esc_attr($k);
                                                                                                        ?>" data-ques="<?php
                                        echo  esc_attr($key);
                            ?>"><span class="dashicons dashicons-trash"></span></a>
                                ...
                                <div class="section-action-btn">
                                    ...
                                    <input type="button" name="del_ques" value="Delete" class="ques-del-btn" data-quesid="<?php
                                                                                                                            echo  esc_attr($key);
                                                                                                                            ?>">
                                    <a href="javascript:void(0);" onclick="location.reload();">Cancel</a>
                                    <div class="loader" style="display:none;">
                                        <img class="ajax-loader" src="<?php
                                                                        echo  esc_url(mystaff_training_plugin_dir_folder . '/templates/loading.gif');
                                                                        ?>" />
                                    </div>
                                </div>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery(document).on('click', '#save_ques', function(e) {
            ...
            if (validation == true) {
                ...
                var quesimage = jQuery(this).parents('.que-item').find('input[name="section_icon_attachment[]"]').val();
                ...
                if (result.isConfirmed) {
                    ...
                    jQuery.ajax({
                        ...
                        data: {
                            ...
                            quizid: quizid,
                            sectionid: '<?php
                                        echo  esc_js(sanitize_text_field($_GET['section_id']));
                                        ?>',
                            subsectiontitle: '<?php
                                                echo  esc_js(sanitize_text_field($_GET['sub_sec_title']));
                                                ?>',
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        window.location.href = '<?php
                                                                echo  esc_url(site_url('/wp-admin/admin.php?page=learning_sections&action=edit&section_id='));
                                                                ?>' + id;
    ...
    jQuery(document).on('click', '.section_upload_icon label', function(e) {
        ...
        wp.media.editor.send.attachment = function(props, attachment) {
            ...
            jQuery(' .section_upload_icon label img').attr('src', attachment.url);
            jQuery(' .section_upload_icon label img').addClass('');
        };
